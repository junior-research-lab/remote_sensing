...
```{r SI2}
SI2 <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r SI3}
SI3 <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r SI4}
SI4 <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r SI5}
SI5 <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r SI6}
SI6 <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r BI}
BI <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r NDSI}
NDSI <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r VSSI}
VSSI <- (bandNIR_crop - bandR_crop) / (bandNIR_crop + bandR_crop)
...
```{r modelPCR}
modelPCR <- train(ground_data~., data = train.data, method = "pcr", scale = TRUE, trControl = trainControl("cv", number = 10), tuneLength = 8) 
...
```{r modelPLS}
modelPLS <- train(ground_data~., data = train.data, method = "pcr", scale = TRUE, trControl = trainControl("cv", number = 10), tuneLength = 8) 
# Plot model RMSE vs different values of components
plot(modelPLS)
# Print the best tuning parameter ncomp that
# minimize the cross-validation error, RMSE
...
```{r modelElasticNet}
model <- train(ground_data~., data = train.data, method = "glmnet", trControl = trainControl("cv", number = 10), tuneLength = 10)
